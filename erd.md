...
Use Cases
	•	Picking Recurring Stocks: By identifying symbols that show up across multiple days, we can highlight stocks with consistent positive performance signals.
	•	Price Range Insights: Summary tables and plots show the typical price per share, helping identify whether gainers tend to be low-cap, mid-cap, or high-cap stocks.
	•	Behavior Patterns: Aggregated historical OHLCV data provides clues about price momentum, volatility, and volume trends.
...
Gainer data is scraped daily from Yahoo and WSJ using automated scripts scheduled via cron. The raw CSVs include symbol, price, change, and percent change.
	•	Normalization:
Both sources are normalized into a consistent schema (symbol, price, price_change, price_percent_change) using a shared normalize_csv.py script.
...
For each symbol in the gainers list, a separate CSV containing historical OHLCV data is fetched and summarized to calculate weekly averages.
	•	Intermediate Tables:
We generate structured tables that:
...
The core questions about recurrence and price behavior were answered through a clean pipeline of scraping, normalizing, and aggregating data.
